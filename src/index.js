...
module.exports = snapsave = (url) =>{
  ...
  try {
  if (!url.match(/(?:https?:\/\/(web\.|www\.|m\.)?(facebook|fb)\.(com|watch)\S+)?$/) && !url.match(/(https|http):\/\/www.instagram.com\/(p|reel|tv|stories)/gi)) return resolve({ developer: 'zuye', status: false, msg: `Link Url not valid` })
  ...
  if (!results.length) return resolve({ developer: 'zuye', status: false, msg: `Blank data` })
  return resolve({ developer: 'zuye', status: true, data: results })
  } catch (e) {
  return resolve({ developer: 'zuye', status: false, msg: e.message })
  }
  })
}